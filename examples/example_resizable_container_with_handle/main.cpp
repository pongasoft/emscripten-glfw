...
int main()
{
  ...
  auto window = glfwCreateWindow(600, 400, "Resizable + Handle | emscripten-glfw", nullptr, nullptr);
  if(!window)
    return -1;

  // makes the canvas resizable to the size of its div container
  emscripten_glfw_make_canvas_resizable(window, "#canvas-container", "#canvas-handle");
